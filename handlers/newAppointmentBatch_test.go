...
func TestHandler_NewAppointmentBatch(t *testing.T) {
	r := SetUpRouter()
	testCases := []struct {
		name      string
		bodyBytes []byte
		...
		{
			name: "endpoint should correctly return multiple added appointments",
			bodyBytes: []byte(`[
			    {
			        "ended_at": "2019-01-24T10:30:00-08:00",
			        "id": 2,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			w := sendTestRequest(r, http.MethodPost, PathNewAppointmentBatch, tc.bodyBytes)
